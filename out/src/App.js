...
class App extends BaseApp {
    ...
    async init() {
        ...
        if (Config.commands.global) {
            try {
                await rest.put(Routes.applicationCommands(BaseApp.Client.getClient().user?.id ?? "Unknown"), { body: data });
            }
            catch (e) {
                Logger.internalError("Cannot push global commands to Discord!", "DISCORD ERROR", e.message, e.stack);
            }
        ...
        if (Config.commands.allGuilds) {
            ...
            for (const guild of guilds) {
                try {
                    await rest.put(Routes.applicationGuildCommands(BaseApp.Client.getClient().user?.id ?? "Unknown", guild[1].id), { body: data });
                }
                catch (e) {
                    Logger.internalError(`Cannot push commands to guild ${guild[1].id}!`, "DISCORD ERROR", e.message, e.stack);
                    continue;
                }
            ...
            for (const guild of Config.commands.guilds) {
                try {
                    await rest.put(Routes.applicationGuildCommands(BaseApp.Client.getClient().user?.id ?? "Unknown", guild), { body: data });
                }
                catch (e) {
                    Logger.internalError(`Cannot push commands to guild ${guild}!`, "DISCORD ERROR", e.message, e.stack);
                    continue;
                }
